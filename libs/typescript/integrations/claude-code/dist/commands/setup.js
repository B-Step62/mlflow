...
import { getEffectiveTracingConfig, isValidTrackingUri, resolveExperiment, resolveSettingsPath, writeTracingSettings, } from '../config.js';
...
function askQuestion(label, defaultValue, validate) {
...
async function promptExperimentMode() {
    return selectPrompt({
...
function printSummary(settingsPath, config) {
    ...
    const port = config.trackingUri.startsWith('http://') || config.trackingUri.startsWith('https://')
...
export async function runSetup(args, options = {}) {
    ...
    const merged = { ...parsed, ...options, projectLocal: parsed.projectLocal };
    ...
    const projectLocal = merged.projectLocal !== undefined
        ? merged.projectLocal
        : interactive
            ? await promptScope()
            : true;
    ...
    try {
        const trackingUri = merged.trackingUri ??
            (interactive
        ...
        if (!experimentId && !experimentName) {
            if (interactive) {
                ...
                if (mode === 'id') {
                    ...
                    experimentName = await askQuestion('MLflow experiment name', DEFAULT_EXPERIMENT_NAME);
                }
            }
            else {
        ...
        if (resolvedExperiment.created && resolvedExperiment.experimentName) {
            console.error(`${OK} Created MLflow experiment ${cyan(resolvedExperiment.experimentName)} (${cyan(resolvedExperiment.experimentId)})`);
        }
        else {
...
export async function runStatus(options = {}) {
    ...
    console.error(`  Experiment name: ${config.experimentName ? cyan(config.experimentName) : dim('not set')}`);
    if (!config.enabled) {
