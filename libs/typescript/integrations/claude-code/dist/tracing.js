...
import { startSpan, flushTraces, InMemoryTraceManager, SpanType, SpanAttributeKey, TraceMetadataKey, TokenUsageKey, } from '@mlflow/core';
import { extractTextContent, findFinalAssistantResponse, findLastUserMessageIndex, getNextTimestampNs, parseTimestampToNs, readTranscript, } from './transcript.js';
// ============================================================================
// Constants
// ============================================================================
 ...
 */
function extractContentAndTools(content) {
    let textContent = '';
    const toolUses = [];
 ...
 */
function findToolResults(transcript, startIdx) {
    const results = {};
    ...
    for (let i = startIdx + 1; i < transcript.length; i++) {
        ...
        const entryToolUseResult = entry.toolUseResult && typeof entry.toolUseResult === 'object' ? entry.toolUseResult : {};
 ...
 */
function getInputMessages(transcript, currentIdx) {
    const messages = [];
    for (let i = currentIdx - 1; i >= 0; i--) {
        ...
        if (entry.type === 'assistant' && msg) {
            ...
            if (typeof content === 'string') {
                ...
                hasText = content.some((p) => typeof p === 'object' && p != null && 'type' in p && p.type === 'text');
            }
            if (hasText) {
                break;
            }
...
function setTokenUsageAttribute(span, usage) {
    if (!usage) {
        return;
    }
 ...
 */
function collectSubagentGroups(transcript, startIdx) {
    const groups = {};
 ...
 */
function createAgentWrapperSpan(parentSpan, toolInput, startNs) {
 ...
 */
function createSubagentSpans(parentSpan, group, startNs, totalDurationNs, toolInput) {
 ...
 */
function createSubagentSpansFromFile(parentSpan, subagentTranscriptPath, startNs, totalDurationNs, toolInput) {
    try {
 ...
 */
function createLlmAndToolSpans(parentSpan, transcript, startIdx, transcriptPath) {
    ...
    for (let i = startIdx; i < transcript.length; i++) {
        ...
        if (toolUses.length) {
            ...
            for (let idx = 0; idx < toolUses.length; idx++) {
                ...
                if (subagentPath) {
                    createSubagentSpansFromFile(toolSpan, subagentPath, toolStartNs, toolDurationNs, toolInput);
                }
                else if (subagentGroups[toolUseId]) {
                    createSubagentSpans(toolSpan, subagentGroups[toolUseId], toolStartNs, toolDurationNs, toolInput);
                }
 ...
 */
export async function processTranscript(transcriptPath, sessionId) {
    try {
        ...
        try {
            ...
            if (trace) {
                ...
                const claudeCodeVersion = transcript.reduce((found, entry) => found ?? entry.version, undefined);
