...
[MLflow Tracing](../../) provides automatic tracing capability for Databricks models through the OpenAI SDK integration. Since Databricks serves models via an OpenAI-compatible API format, you can use `mlflow.openai.autolog()` to trace interactions with Databricks models.

:::info What is Databricks?
<u>[Databricks](https://www.databricks.com/)</u> provides DBRX and other foundation models via Model Serving with an OpenAI-style REST interface.
:::

:::tip Why use MLflow with Databricks?

MLflow provides comprehensive observability for your applications using Databricks-hosted LLMs. Traces are also seamlessly integrated with MLflow's prompt management, evaluation, and monitoring capabilities.
...
<StepHeader number={3} title="Enable tracing and call Databricks" />
...
<Tabs>
  <TabItem value="python" label="Python" default>
    ...
    # Create a parent span for the Databricks call
  ...
  <TabItem value="typescript" label="JS / TS">
    ...
    // Create a traced function that wraps the Databricks call
...
Running either example will produce a trace that includes the Databricks LLM span; the traced function creates the parent span automatically.
