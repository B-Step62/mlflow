...
<u>[Ollama](https://ollama.com/)</u> is an open-source platform that enables users to run large language models (LLMs) locally on their devices, such as Llama 3.2, Gemma 2, Mistral, Code Llama, and more.
:::

:::tip Why use MLflow with Ollama?
MLflow gives you full observability—traces, token usage, latency, and metadata—while integrating with prompt management, evaluation, and user feedback, all for your locally running models.
:::
