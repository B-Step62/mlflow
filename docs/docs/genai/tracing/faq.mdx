...
This section answers frequently asked questions about [MLflow Tracing](/genai/tracing).

## Instrumentation

### Q: Which libraries does MLflow Tracing support automatic instrumentation?
...
### Q: Where can I request a new library integration?

If you don't see your favorite library in the list, you can [contribute](/genai/tracing/integrations/contribute) or [submit a feature request](https://github.com/mlflow/mlflow/issues/new?assignees=&labels=enhancement&projects=&template=feature_request_template.yaml&title=%5BFR%5D) to our Github repository.
...
### Q: How can I turn on/off in-line trace UI in Jupyter notebooks?
...
### Q: `A trace with ID 'tr-...' already exists` error is thrown when starting a trace

There can be several reasons for this error, but a common one is that you are setting seed for a RNG via `random.seed()`. MLflow Tracing depends on OpenTelemetry to generate unique trace IDs. The generation logic uses the `random` library and becomes a fixed sequence when you fix the seed. See [this Github issue](https://github.com/open-telemetry/opentelemetry-python/issues/2974) for more details.

To workaround this, you can unset the random seed. Note that `numpy.random.seed()` does not have this issue.
...
You can delete traces using the <APILink fn="mlflow.MlflowClient.delete_traces" /> method:
