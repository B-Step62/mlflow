...
describe('API', () => {
  ...
  describe('withSpan', () => {
    ...
    describe('edge cases', () => {
      ...
      it.skip('should handle multiple async tasks with proper parent-child relationships', async () => {
        ...
        // TODO: Fix test - need to mock HTTP client or start server
        // const traces = getTraces();
        // expect(traces.length).toBe(3);

        // TODO: This test needs to be redesigned to handle multiple traces properly
        /*
        // Verify first trace (most complex with grandchild)
        const trace1 = traces.find((t: any) => t.data.spans.some((s: any) => s.name === 'trace1-parent'));
        expect(trace1).toBeDefined();
        expect(trace1!.data.spans.length).toBe(4); // parent + 2 children + 1 grandchild

        const trace1Parent = trace1!.data.spans.find((s: any) => s.name === 'trace1-parent');
        const trace1Child1 = trace1!.data.spans.find((s: any) => s.name === 'trace1-child1');
        const trace1Child2 = trace1!.data.spans.find((s: any) => s.name === 'trace1-child2');
        const trace1Grandchild = trace1!.data.spans.find((s: any) => s.name === 'trace1-grandchild');
        ...
        // Verify second trace
        const trace2 = traces.find((t: any) => t.data.spans.some((s: any) => s.name === 'trace2-parent'));
        expect(trace2).toBeDefined();
        expect(trace2!.data.spans.length).toBe(2); // parent + 1 child

        const trace2Parent = trace2!.data.spans.find((s: any) => s.name === 'trace2-parent');
        const trace2Child = trace2!.data.spans.find((s: any) => s.name === 'trace2-child');
        ...
        const trace3 = traces.find((t: any) => t.data.spans.some((s: any) => s.name === 'trace3-simple'));
