...
export function startSpan(options: SpanOptions): LiveSpan {
    try {
        ...
        const otelSpan = tracer.startSpan(options.name, {startTime: startTime}, parentContext) as OTelSpan;
...
export function withSpan<T>(
    ...
    const spanOptions: Omit<SpanOptions, 'parent'> = isOptionsProvided ? optionsOrCallback : {name: DEFAULT_SPAN_NAME};
    const actualCallback = isOptionsProvided ? (callback ?? (() => { throw new Error('Callback is required when options are provided'); })) : optionsOrCallback;
...
function createAndRegisterMlflowSpan(
    ...
    const traceId = InMemoryTraceManager.getInstance().getMlflowTraceIdFromOtelId(otelTraceId) || otelTraceId;
