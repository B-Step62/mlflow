...
export class DatabricksArtifactsClient implements ArtifactsClient {
  ...
  private async getCredentialsForTraceDataUpload(
    ...
    const response = await makeRequest<GetCredentialsForTraceDataUpload.Response>('GET', url, getRequestHeaders(this.token));
  ...
  private async getCredentialsForTraceDataDownload(
    ...
    const response = await makeRequest<GetCredentialsForTraceDataDownload.Response>('GET', url, getRequestHeaders(this.token));
   ...
   */
  private async downloadFromSignedUrl(credentials: ArtifactCredentialInfo): Promise<SerializedTraceData> {
    const headers: Record<string, string> = {};
