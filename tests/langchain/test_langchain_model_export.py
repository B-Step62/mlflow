...
def test_save_and_load_chat_openai(model_path):
    ...
    prediction = loaded_pyfunc_model.predict(["Mlflow?"])
    # Response from ChatOpenAI should be AIMessage instance with 'content' attribute
    assert prediction[0].content == TEST_CONTENT
...
def test_save_and_load_llm_chain_with_chat_openai(model_path):
    from langchain.chat_models import ChatOpenAI
...
def test_save_and_load_llm_chain_with_chat_openai_with_unsupported_version_raise_helpful_message(
    model_path,
):
    from langchain.chat_models import ChatOpenAI
...
def test_save_and_load_llm_chain__with_azure_chat_openai(model_path):
