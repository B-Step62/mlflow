...
def multi_modal_pipeline(component_multi_modal):
    ...
    pipeline = _build_pipeline_from_model_input(
        _TransformersModel.from_dict(**component_multi_modal), task
    )
...
def test_flavor_config_tf(small_seq2seq_pipeline):
    ...
    conf = build_flavor_config(small_seq2seq_pipeline, "text-classification")
...
def test_flavor_config_pt_save_pretrained_false(small_qa_pipeline):
    ...
    conf = build_flavor_config(small_qa_pipeline, "question-answering", save_pretrained=False)
...
def test_flavor_config_component_multi_modal(multi_modal_pipeline):
    ...
    conf = build_flavor_config(pipeline, task, processor)
...
def test_flavor_config_component_multi_modal_save_pretrained_false(multi_modal_pipeline):
    ...
    conf = build_flavor_config(pipeline, task, processor, False)
