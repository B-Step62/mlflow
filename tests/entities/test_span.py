...
def test_create_live_span():
    ...
    with tracer.start_as_current_span("parent") as parent_span:
        ...
        assert span._otel_trace_id == encode_trace_id(parent_span.context.trace_id)
        ...
        with tracer.start_as_current_span("child") as child_span:
            span = create_mlflow_span(child_span, trace_id)
            assert isinstance(span, LiveSpan)
...
def test_create_non_live_span():
    ...
    assert span.trace_id == trace_id
    assert span._otel_trace_id == encode_trace_id(12345)
...
def test_create_noop_span():
    ...
    # create from None
    span = create_mlflow_span(None, trace_id)
    assert isinstance(span, NoOpSpan)
...
def test_dict_conversion():
    ...
    tracer = _get_tracer("test")
    with tracer.start_as_current_span("parent") as parent_span:
        span = LiveSpan(parent_span, trace_id=trace_id, span_type=SpanType.LLM)

    span_dict = span.to_dict()
    recovered_span = Span.from_dict(span_dict)

    assert span.trace_id == recovered_span.trace_id
    assert span._otel_trace_id == recovered_span._otel_trace_id
