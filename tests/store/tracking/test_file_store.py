...
def test_end_trace(store_and_trace_info):
    ...
    with pytest.raises(MlflowException, match=r"Trace with request ID 'fake_trace_id' not found"):
...
def test_search_traces_filter(generate_trace_infos):
    ...
    _validate_search_traces(store, [exp_id], f"trace_id != '{trace_ids[0]}'", trace_infos[1:][::-1])
    _validate_search_traces(store, [exp_id], f"trace_id IN ('{trace_ids[0]}')", [trace_infos[0]])
    _validate_search_traces(
        store,
        [exp_id],
