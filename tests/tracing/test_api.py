...
def mock_client():
    ...
    with mock.patch("mlflow.tracing.api.TracingClient", return_value=client):
        yield client
...
def test_get_trace(mock_get_display_handler):
    ...
    with mock.patch("mlflow.tracing.api._logger") as mock_logger:
...
def test_search_traces(return_type, mock_client):
    if _IS_TRACE_PACKAGE_TEST and return_type == "pandas":
        pytest.skip("Skipping test because it requires full MLflow package.")
...
def test_search_traces_with_pagination(mock_client):
    ...
    traces = mlflow.search_traces(
        experiment_ids=["1"],
        return_type="list" if _IS_TRACE_PACKAGE_TEST else "pandas"
    )
...
def test_search_traces_with_default_experiment_id(mock_client):
    mock_client.search_traces.return_value = PagedList([], token=None)
    with mock.patch("mlflow.tracing.api._get_experiment_id", return_value="123"):
        mlflow.search_traces(return_type="list" if _IS_TRACE_PACKAGE_TEST else "pandas")
...
def test_search_traces_handles_missing_response_tags_and_metadata(mock_client):
    ...
    df = mlflow.search_traces(return_type="list" if _IS_TRACE_PACKAGE_TEST else "pandas")
...
@skip_if_not_full_mlflow
def test_search_traces_with_invalid_span_content(monkeypatch, mock_client):
    mock_client.search_traces.return_value = PagedList([
        Trace(
            info=TraceInfo(
                request_id=5,
