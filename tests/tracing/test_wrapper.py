...
import mlflow
from mlflow.entities import Span, SpanStatus, SpanType, TraceStatus
...
def test_wrap_active_span(clear_singleton):
    ...
    with tracer.start_as_current_span("parent") as parent_span:
        ...
        span.set_status("OK")
        assert span.status == SpanStatus(TraceStatus.OK)
...
def test_wrap_non_active_span():
    ...
    assert span.outputs == 2
    assert span.status == SpanStatus(TraceStatus.UNSPECIFIED, description="")
    assert span.get_attribute("key") == 3
