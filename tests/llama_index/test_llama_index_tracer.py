...
from mlflow.llama_index.tracer import MlflowSpanHandler
...
def test_trace_llm_chat(is_async):
    ...
    attr = spans[0].attributes
    # NB: The span type will be inferred CHAT_MODEL once we add an event handler in next PR
    assert attr[SpanAttributeKey.SPAN_TYPE] == SpanType.LLM
...
def test_trace_llm_error():
    llm = OpenAI(
        api_base="http://localhost:1234/invalid/base",
        timeout=1,
        max_retries=0,
    )
    assert llm.api_base == "http://localhost:1234/invalid/base"
    ...
    # NB: The span type will be inferred CHAT_MODEL once we add an event handler in next PR
    assert spans[0].attributes[SpanAttributeKey.SPAN_TYPE] == SpanType.LLM
