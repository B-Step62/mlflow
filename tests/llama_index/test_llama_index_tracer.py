...
async def test_tracer_parallel_workflow_with_custom_spans():
    ...
    class ParallelWorkflow(Workflow):
        ...
        async def process_data(self, ev: ProcessEvent) -> ResultEvent:
            # Simulate some time-consuming processing
            await asyncio.sleep(random.randint(1, 2))
            with mlflow.start_span(name="custom_inner_span_worker") as span:
                pass
            return ResultEvent(result=ev.data)
    ...
    with mlflow.start_span(name="custom_root_span") as span:
    ...
    root_span = spans[0]
    assert root_span.name == "custom_root_span"
    assert root_span.inputs == inputs
    assert root_span.outputs == result
