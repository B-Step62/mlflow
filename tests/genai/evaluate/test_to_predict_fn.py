...
def test_to_predict_fn_thread_safe(sample_rag_trace, mock_deploy_client, mock_tracing_client):
    # Create a copy every time because "databricks_output" is popped in each call
    ...
    mock_deploy_client.predict.side_effect = _mock_predict
    messages = [
        {"content": "You are a helpful assistant.", "role": "system"},
        {"content": "What is Spark?", "role": "user"},
    ]
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(predict_fn, messages=messages) for _ in range(100)]
        for future in as_completed(futures):
            response = future.result()
            assert response == _DUMMY_CHAT_RESPONSE  # Response should not contain databricks_output

    mock_deploy_client.predict.assert_has_calls(
        [
            mock.call(
                endpoint="chat",
                inputs={"messages": messages, "databricks_options": {"return_trace": True}},
            )
            for _ in range(100)
        ]
    )
    ...
    # Trace from endpoint (sample_rag_trace) should be copied to the current experiment
    mock_tracing_client.start_trace_v3.assert_has_calls([mock.call(mock.ANY) for _ in range(100)])
    for call in mock_tracing_client.start_trace_v3.call_args_list:
        trace = call[0][0]
        # Copied trace should have a new trace ID
        assert trace.info.trace_id != sample_rag_trace.info.trace_id
        assert trace.info.request_preview == '{"question": "query"}'
