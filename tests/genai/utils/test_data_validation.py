...
def test_check_model_prediction_unmatched_keys_with_variable_kwargs():
    ...
    with pytest.raises(MlflowException, match=r"Failed to run the prediction function") as e:
        check_model_prediction(fn, sample_input)

    # When the function contains kwargs, it is hard to inspect the cause of error.
    # So we show a generic example
    expected_code_example = """
def predict_fn(question, context):
    ...
...
data = [
    {
        "inputs": {
            ...
            "context": "MLflow is a platform for ...",
        }
    }
]
...
def test_check_model_prediction_unknown_error():
    ...
    # Show generic example when the error is unknown
    expected_code_example = """
def predict_fn(question, context):
    ...
