...
def test_validate_scorers_legacy_metric():
    ...
    assert "@metric" in mock_logger.warning.call_args[0][0]
...
def test_validate_data_with_expectations():
    ...
    data = pd.DataFrame(
        {
            ...
            "retrieved_context": ["context1", "context2"],
            "expectations": [
                {"expected_response": "expectation1"},
                {"expected_response": "expectation2"},
            ],
        }
    )
...
def test_validate_data_with_trace():
    ...
    traces = mlflow.search_traces(return_type="pandas", order_by=["timestamp_ms ASC"])
    data = traces[["trace"]]
