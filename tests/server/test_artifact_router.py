...
def test_download_does_not_hit_legacy_handler(client, tmp_path):
    test_file = tmp_path / "model.pkl"
    test_file.write_bytes(b"test")

    with (
        mock.patch("mlflow.server.artifact_router._get_artifact_repo") as mock_get_repo,
        mock.patch("mlflow.server.handlers._download_artifact") as mock_legacy_handler,
    ):
        mock_repo = mock.MagicMock()
        mock_repo.get_local_path.return_value = str(test_file)
        mock_get_repo.return_value = mock_repo
...
def test_upload_does_not_hit_legacy_handler(client):
    with (
        mock.patch("mlflow.server.artifact_router._get_artifact_repo") as mock_get_repo,
        mock.patch("mlflow.server.handlers._upload_artifact") as mock_legacy_handler,
    ):
        mock_repo = mock.MagicMock(spec=ArtifactRepository)
        mock_get_repo.return_value = mock_repo
