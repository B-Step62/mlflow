...
def get_trace(trace_id: str) -> Optional[Trace]:
    """
    Get a trace by the given trace_id ID if it exists.
...
def get_current_active_span() -> Optional[LiveSpan]:
    ...
    trace_id = json.loads(otel_span.attributes.get(SpanAttributeKey.REQUEST_ID))
    return trace_manager.get_span_from_id(trace_id, encode_span_id(otel_span.context.span_id))
...
def update_current_trace(
    ...
    trace_id = active_span.reqtrace_iduest_id
...
def _merge_trace(
    ...
    Args:
        ...
        target_trace_id: The request ID of the parent trace.
    ...
    with trace_manager.get_trace(target_trace_id) as parent_trace:
        if not parent_trace:
            _logger.warning(
                f"Parent trace with request ID {target_trace_id} not found. Skipping merge."
            )
            return
