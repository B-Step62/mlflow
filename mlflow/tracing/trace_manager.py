...
class InMemoryTraceManager:
    ...
    def start_detached_span(
        ...
        try:
            ...
            if not request_id:
                request_id = self.create_empty_trace(
    ...
    def create_empty_trace(
        ...
        try:
            trace_info = MlflowClient()._tracking_client.start_trace(
            ...
            _logger.warning(f"Failed to start trace in backend, the API might not be available: {e}."
                            "Falling back to local trace.")
