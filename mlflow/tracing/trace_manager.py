...
class InMemoryTraceManager:
    ...
    def __init__(self):
        ...
        self._otel_trace_id_to_mlflow_id: dict[int, str] = {}
    ...
    def register_trace(self, otel_trace_id: int, trace_info: TraceInfo):
        ...
        with self._lock:
            ...
            self._otel_trace_id_to_mlflow_id[otel_trace_id] = trace_info.trace_id
    ...
    def update_trace_info(self, trace_info: TraceInfo):
        ...
        """
        with self._lock:
            if trace_info.trace_id not in self._traces:
                _logger.debug(f"Trace data with request ID {trace_info.trace_id} not found.")
                return
            self._traces[trace_info.trace_id].info = trace_info
    ...
    def get_span_from_id(self, trace_id: str, span_id: str) -> Optional[LiveSpan]:
        """
        Get a span object for the given trace and span ID.
        """
        with self._lock:
    ...
    def get_mlflow_trace_id_from_otel_id(self, otel_trace_id: int) -> Optional[str]:
        ...
        """
        return self._otel_trace_id_to_mlflow_id.get(otel_trace_id)

    def set_request_metadata(self, trace_id: str, key: str, value: str):
        """
        Set the request metadata for the given trace ID.
        """
    ...
    def pop_trace(self, otel_trace_id: int) -> Optional[Trace]:
        ...
        """
        with self._lock:
            trace_id = self._otel_trace_id_to_mlflow_id.pop(otel_trace_id, None)
