    ...
    if code_paths is not None:
        ...
        for code_path in code_paths:
            try:
                ...
                raise MlflowException(
                    message=(
                        f"Failed to copy the specified code path '{code_path}' into the model artifacts.",
                        "It appears that your code path includes file(s) that cannot be copied, ",
                        "such as Databricks Notebooks. Please specify a code path that does not ",
                        "include such files and try again.",
                    ),
