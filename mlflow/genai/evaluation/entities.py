...
from mlflow.genai.evaluation.agent_utils import StrEnum
from mlflow.genai.evaluation.config import GUIDELINE_ADHERENCE, AssessmentType, get_builtin_assessment_config_with_service_assessment_name
from mlflow.genai.evaluation.input_output_utils import is_none_or_nan, normalize_to_dictionary, parse_variant_data, response_to_string
from mlflow.genai.evaluation.schemas import CUSTOM_INPUTS_COL, EXPECTED_RETRIEVED_CONTEXT_COL, GUIDELINES_CONTEXT_COL, INPUTS_COL, REQUEST_COL, RETRIEVED_CONTEXT_COL
...
class CategoricalRating(StrEnum):
    ...
    @classmethod
    def from_example_rating(cls, rating) -> "CategoricalRating":
...
class EvalItem:
    ...
    def raw_guidelines(self):
        ...
        is_named_guidelines = not (
            ...
            and GUIDELINE_ADHERENCE.assessment_name
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "EvalItem":
        ...
        if not raw_request:
            ...
            if isinstance(raw_request, str):
                try:
                    ...
                    raise ValueError(
                        f"`{schemas.INPUTS_COL}` must be JSON serializable: {type(raw_request)}"
                    ) from e
        try:
            question = request_to_string(raw_request)
        except ValueError:
            question = None
        ...
        if not raw_response:
            ...
            raw_response = parse_variant_data(
            ...
            if isinstance(raw_response, str):
                try:
                    ...
                    raise ValueError(
                        f"`{schemas.OUTPUTS_COL}` must be JSON serializable: {type(raw_response)}"
                    ) from e
        try:
            answer = response_to_string(raw_response)
        except ValueError:
            answer = None

        ground_truth_answer = response_to_string(
        ...
        expected_facts = (
            list(expected_facts)
            if not is_none_or_nan(expected_facts)
            else None
        )
        ...
        custom_expected = normalize_to_dictionary(
        ...
        if is_none_or_nan(named_guidelines):
            guidelines = None
            ...
            named_guidelines = {
                GUIDELINE_ADHERENCE.assessment_name: list(
                    named_guidelines
                )
            }

        guidelines_context = data.get(GUIDELINES_CONTEXT_COL)
        guidelines_context = (
            guidelines_context
            if not is_none_or_nan(guidelines_context)
            else None
        )

        custom_inputs = None
        if isinstance(raw_request, dict):
            custom_inputs = raw_request.get(CUSTOM_INPUTS_COL)
        ...
        if is_none_or_nan(source_id):
            source_id = None
        source_type = data.get(schemas.SOURCE_TYPE_COL)
        if is_none_or_nan(source_type):
            source_type = None

        if not source_id:
            source = data.get(schemas.SOURCE_COL)
            if not is_none_or_nan(source):
...
class PerChunkAssessmentResult(AssessmentResult):
    ...
    assessment_type: AssessmentType = dataclasses.field(
        init=False, default=AssessmentType.RETRIEVAL
    )
    span_id: Optional[str] = None
    ...
    def get_assessment_results_dict(self) -> Dict[str, schemas.ASSESSMENT_RESULT_TYPE]:
        ...
        for assessment in self.assessment_results:
            ...
            if (
                ...
                and assessment_type == AssessmentType.RETRIEVAL_LIST
            ):
