        ...
        with mlflow.start_span(
            ...
            if retrieval_context is not None:
                with mlflow.start_span(
                    name="retrieval_span",
                    attributes={
                        mlflow_tracing_constant.SpanAttributeKey.SPAN_TYPE: mlflow_entities.SpanType.RETRIEVER
                    },
                ) as retrieval_span:
...
def inject_experiment_run_id_to_trace(
    ...
    if run_id is not None:
        trace.info.trace_metadata[
            mlflow_tracing_constant.TraceMetadataKey.SOURCE_RUN
        ] = run_id
    ...
    if trace_location.type == mlflow_entities.TraceLocationType.MLFLOW_EXPERIMENT:
        if trace_location.mlflow_experiment is not None:
            ...
            trace.info.trace_location.mlflow_experiment = (
                mlflow_entities.MlflowExperimentLocation(experiment_id=experiment_id)
            )
    else:
        raise ValueError(
            f"Cannot inject experiment ID to trace location type: {trace_location.type}"
        )
...
def clone_trace_to_reupload(trace: mlflow_entities.Trace) -> mlflow_entities.Trace:
    ...
    """
    prepared_trace = deserialize_trace(serialize_trace(trace))
