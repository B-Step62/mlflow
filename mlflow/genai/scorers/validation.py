...
def validate_scorers(scorers: list[Any]) -> tuple[list[BuiltInScorer], list[Scorer]]:
    ...
    if has_legacy_metric:
        _logger.warning(
            "Custom metrics definition using @metric decorator and will soon be deprecated "
            "in future releases. Please use the @scorer decorator instead."
        )
...
def valid_data_for_builtin_scorers(
    ...
    if "trace" in input_columns:
        # Inputs, outputs, retrieved_context are inferred from the trace.
        ...
        input_columns |= {"inputs", "outputs", "retrieved_context"}

        sample_trace = data["trace"].iloc[0]
        if isinstance(sample_trace, str):
            sample_trace = Trace.from_json(sample_trace)
        elif not isinstance(sample_trace, Trace):
            raise MlflowException.invalid_parameter_value(
                "The trace column must be a string or a Trace object"
            )
