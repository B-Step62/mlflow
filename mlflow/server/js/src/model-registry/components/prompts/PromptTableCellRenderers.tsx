...
export const PromptListTagsCell = ({ tags }: { tags: KeyValueEntity[] }) => {
  ...
  const noValue = (
    <em>
      <FormattedMessage description="Prompts table > tags column > no value" defaultMessage="(empty)" />
    </em>
  );
      ...
      {tagsToDisplay.map((tag) => (
        ...
        >
          <div
            ...
            data-testid="prompts-table-tag-entry"
          >
...
/**
 * Renders prompt version with the link in the models table
 */
export const PromptsListVersionLinkCell = ({ versionNumber, name }: { versionNumber?: string; name: string }) => {
  if (!versionNumber) {
    return <EmptyCell />;
  }
  return (
    <FormattedMessage
      ...
      description="Row entry for version columns in the registered prompt page"
      values={{
        versionNumber,
        link: (text: any) => <Link to={ModelRegistryRoutes.getPromptVersionPageRoute(name, versionNumber)}>{text}</Link>,
      }}
    />
  );
};
