...
import { isSyntheticRoleName } from '../account/types';
import type {
    ...
    queryFn: async () => {
      ...
      return {
        ...
        users: data.users.map((u) => ({
          ...u,
          roles: u.roles?.filter((r) => !isSyntheticRoleName(r.name)),
        })),
      };
    },
    retry: false,
...
export const useRolesQuery = (workspaces?: string | readonly string[], options: { enabled?: boolean } = {}) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return { ...data, roles: data.roles.filter((r) => !isSyntheticRoleName(r.name)) };
    },
    retry: false,
    refetchOnWindowFocus: false,
