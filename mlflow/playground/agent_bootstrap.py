...
def _build_app():
    ...
    Called at module import time when ``MLFLOW_PLAYGROUND_AGENT_REPO_DIR``
    is set — i.e. inside the uvicorn worker. Each reload re-imports this
    module and re-runs discovery, so edits to the agent file land in the
    next worker.
    """
...
# imports (tests, IDE tooling) stay side-effect-free because the env var
# is only set by `main()` before exec'ing uvicorn.
