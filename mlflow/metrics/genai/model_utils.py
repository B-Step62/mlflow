...
def get_endpoint_type(endpoint_uri: str) -> Optional[str]:
    ...
    """
    schema, _ = _parse_model_uri(endpoint_uri)
    ...
    endpoint = client.get_endpoint(endpoint_uri)
...
def score_model_on_prompt(
    ...
    if prefix == "openai":
        return _call_openai_api(suffix, prompt, eval_parameters)
    elif prefix == "gateway":
        return _call_gateway_api(suffix, prompt, eval_parameters)
    elif prefix == "endpoints":
        return call_deployments_api(suffix, prompt, eval_parameters, endpoint_type)
...
def _call_openai_api(openai_uri: str, prompt: str, eval_parameters: Dict[str, Any] = None):
