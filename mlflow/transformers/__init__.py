...
def save_model(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    _validate_transformers_task_type(transformers_task)
...
def persist_pretrained_model(model_uri: str) -> None:
    ...
    This API is primary for updating an MLflow Model logged/saved with save_pretrained=False,
    which cannot be registered to the Model Registry, due to the lack of actual pretrained model
    weights in the artifacts but only the reference to the HuggingFace Hub repository. This API
    will download the model weights from the HuggingFace Hub repository and save them in the
    artifacts of the given model_uri, so that the model can be registered to the Model Registry.
    ...
    _logger.info(
        f"The pretrained model has been successfully persisted to the model_uri {model_uri}. Now "
        "you can register the model to the Model Registry via mlflow.register_model() API or UI."
    )
