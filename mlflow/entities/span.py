...
class Span:
    ...
    def request_id(self) -> str:
        """
        DEPRECATED: The unique identifier for a trace was named as "request_id" in
        MLflow 2.x, but it is now renamed to "trace_id". This property is kept for
        backward compatibility, but it will be removed in the future versions.

        # TODO: Undocument this
        """
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Span":
        ...
        """
        try:
            trace_id = data.get("attributes", {}).get(SpanAttributeKey.REQUEST_ID)
            if not trace_id:
                raise MlflowException(
...
NO_OP_SPAN_TRACE_ID = "MLFLOW_NO_OP_SPAN_REQUEST_ID"
